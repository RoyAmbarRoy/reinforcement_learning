        ...
        for is_deep_v in value_deep:
            for lr_p in lr_list_policy:
                for lr_v in lr_list_value:
                    ...
                    target_env_scope = fine_tune_env_name if experiment == 'finetune' else env_name
                    ...
                    policy_networks_list, value_networks_list = getNetworks(state_size,
                                                                            action_size,
                                                                            lr_p,
                                                                            lr_v,
                                                                            is_deep_p,
                    ...
                    policy_network, value_network = policy_networks_list[0], value_networks_list[0]
                    ...
                    with tf.Session() as sess:
                        ...
                        # print('\nActor-Critic last w  before load\n')
                        ...
                        if path_load:
                            if experiment == 'finetune':
                                ...
                                path_conc = c_utils.buildPath(path_load, p_net_deepness, v_net_deepness, lr_p, lr_v)
                                m_loaders.loadModel(path_conc, sess, saver)

                                # print('\nActor-Critic last w  after load\n')
                                # print(sess.run(policy_network.W4))
                                # print(sess.run(value_network.W4))

                                # Re-initialize last layer weights
                                m_loaders.initializeLastLayer(sess, networks_names, verbose=False)

                                # print('\nActor-Critic last w  after initialize\n')
                                # print(sess.run(policy_network.W4))
                                # print(sess.run(value_network.W4))
